...
class EmployeeSchema(BaseModel):
    ...
    date_time: datetime | None = Field(..., description="Date and Time of Hire", examples=["2021-09-01 09:00:00"])
    ...
    def validate_data(cls, v: dict) -> dict:
        ...
        if v["name"] is None or isinstance(v["name"], float):
            v["name"] = None
        if not isinstance(v["date_time"], datetime) and not isinstance(v["date_time"], float):
            v["date_time"] = datetime.fromisoformat(v["date_time"])
        if isinstance(v["date_time"], float):
            v["date_time"] = None
        if v["department_id"] is None or v["department_id"] < 0 or math.isnan(v["department_id"]):
